"""Project frozen validation latent spaces with deterministic exact Torch t-SNE."""
...
_SCHEMA = "methylation-latent.validation-latent-tsne.v1"
...
def _load_tuning_model(
    ...
    *,
    ...
    if (
        ...
        raise ValueError(f"t-SNE tuning-model identity differs: {directory}")
    model = LatentMetric(
...
def _context_labels(
    ...
    # Kept separate so the balancing contract is explicit at the one metadata-access seam.
...
def main() -> None:
    ...
    if config.status != "frozen":
        raise ValueError("latent t-SNE requires the frozen primary protocol")
    ...
    if (
        ...
        raise ValueError("t-SNE windows must be increasing members of the frozen sweep")
    if (
        not dimensions
        or tuple(sorted(set(dimensions))) != dimensions
        or any(dimension > 128 for dimension in dimensions)
    ):
        raise ValueError("t-SNE dimensions must be unique, increasing, and at most 128")
    ...
    for split_offset, split_name in enumerate(_SPLITS):
        ...
        if not t.equal(
            ...
            raise RuntimeError("t-SNE display sample is not exactly context-balanced")
        ...
        for window in windows:
            ...
            for dimension in dimensions:
                ...
                with t.inference_mode():
                    latent = model.latent(display_embeddings).to(t.float64)
                ...
                if not t.allclose(
                    ...
                    raise RuntimeError("t-SNE input latent rows are not unit norm")
                projection = exact_tsne(latent, config=_CONFIG)
                ...
                record: dict[str, JsonValue] = {
                    ...
                    "sampling": {
                        ...
                        "display_count": _MAXIMUM_DISPLAY_POINTS,
                        "count_per_context": expected_context_count,
                    },
                    "tsne": {
                        "implementation": "exact_torch_student_t",
                        "input_geometry": "euclidean_on_unit_latent_rows_equivalent_to_cosine",
                        "shared_initialization_across_panels": True,
                        "config": cast(dict[str, JsonValue], asdict(_CONFIG)),
                        "final_kl_divergence": projection.final_kl_divergence,
                    },
                ...
                if output_path.is_file():
                    if _load_json(output_path) != record:
                        raise ValueError(f"existing t-SNE output differs: {output_path}")
                else:
                    write_canonical_json_exclusive(output_path, record)
                print(
                    f"tsne split={split_name} window={window} d={dimension} "
                    f"lambda={_LAMBDA_AGE:g} points={len(points)} "
                    f"kl={projection.final_kl_divergence:.8f} output={output_path}"
                )
